--!strict
--[[
    SACRAMENT | Combat Orchestrator (init)
    Responsável pela composição e gerenciamento de estado da aba Combat.
--]]
...
-- Sub-módulos (Paths preservados conforme a nova estrutura)
...
-- Constantes Visuais (Identidade Sacrament)
...
local function createHitboxOverGlyph(maid: any, glyph: Instance): GuiButton?
    ...
    hit.Text = ""
    hit.ZIndex = guiObj.ZIndex + 5 -- Garante prioridade de clique
...
function CombatModuleFactory.new(): CombatModule
    ...
    local function bindAccordion(item: AccordionItem)
        ...
        local function onToggle()
            -- RESOLUÇÃO DO BUG: Checamos a visibilidade real da SubFrame

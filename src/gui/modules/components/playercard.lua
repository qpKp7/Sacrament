...
export type PlayerCardUI = {
    Instance: Frame,
    Destroy: (self: PlayerCardUI) -> ()
}

local PlayerCardFactory = {}
...
function PlayerCardFactory.new(layoutOrder: number): PlayerCardUI
    local maid = Maid.new()
    ...
    -- O Layout mestre da Box agora é Horizontal
    ...
    -- 1. Avatar (Quadrado 1:1, Corpo Inteiro)
    local avatarSize = 100
    local avatar = Instance.new("ImageLabel")
    avatar.Name = "Avatar"
    avatar.Size = UDim2.new(0, avatarSize, 0, avatarSize)
    -- type=Avatar puxa o personagem por completo
    ...
    -- 2. Container de Textos (Lado Direita)
    local textContainer = Instance.new("Frame")
    textContainer.Name = "TextContainer"
    -- Usa o espaço horizontal restante e alinha ao centro no eixo Y
    textContainer.Size = UDim2.new(1, -(avatarSize + 15), 1, 0)
    textContainer.BackgroundTransparency = 1
    textContainer.LayoutOrder = 2
    textContainer.Parent = box
    ...
    local function createText(name: string, text: string, size: number, color: Color3, order: number, font: Enum.Font)
        ...
        lbl.Font = font
        lbl.TextSize = size
        lbl.TextXAlignment = Enum.TextXAlignment.Left -- Rigorosamente a esquerda

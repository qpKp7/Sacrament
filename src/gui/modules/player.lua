...
function PlayerModuleFactory.new(): PlayerModule
    ...
    local function applyState(item: AccordionItem, open: boolean, animate: boolean)
        ...
        if open then
            ...
            -- Corrige o bug de renderização: Força recálculo do ScrollingFrame e UIListLayout ao abrir
            task.defer(function()
                for _, desc in ipairs(item.subFrame:GetDescendants()) do
                    if desc:IsA("ScrollingFrame") then
                        ...
                        if layout then
                            ...
                            -- Garante que o recálculo continue se os componentes mudarem dinamicamente
    ...
    local function registerAccordion(header: Frame, subFrame: Frame, layoutOrder: number)
        ...
        subFrame.ClipsDescendants = true -- Blindagem extra para evitar vazamento visual

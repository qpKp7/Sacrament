...
function AntiStunFactory.new(layoutOrder: number?): AntiStunUI
    ...
    container.Name = "AntiStunContainer"
    ...
    local isEnabled = UIState and UIState.Get("AntiStunEnabled", false) or false
    ...
    -- EVENTO DO TOGGLE PRINCIPAL DO ANTISTUN
    if toggleBtn and UIState then
        maid:GiveTask(toggleBtn.Toggled:Connect(function(state: boolean)
            ...
            UIState.Set("AntiStunEnabled", state)
        end))
    end
    ...
    -- 🎯 PAINEL DE CONTROLE DE MEMÓRIA DO ANTISTUN
    Orchestrator.Bind(secKeybind, "AntiStun_Keybind", "Keybind")
    Orchestrator.Bind(secKeyHold, "AntiStun_KeyHold", "Toggle")
    -- =========================================================================

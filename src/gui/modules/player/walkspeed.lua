...
function WalkSpeedFactory.new(layoutOrder: number?): WalkSpeedUI
    ...
    local isEnabled = UIState and UIState.Get("WalkSpeedEnabled", false) or false
    ...
    local secSpeed   = loadSec(SpeedSection, 2, inputsScroll)
    local secTrail   = loadSec(TrailSection, 3, inputsScroll)

    -- EVENTO DO TOGGLE PRINCIPAL DO WALKSPEED
    if toggleBtn and UIState then
        maid:GiveTask(toggleBtn.Toggled:Connect(function(state: boolean)
            ...
            UIState.Set("WalkSpeedEnabled", state)
        end))
    end
    ...
    -- 🎯 PAINEL DE CONTROLE DE MEMÓRIA DO WALKSPEED
    Orchestrator.Bind(secKeybind, "WalkSpeed_Keybind",       "Keybind")
    Orchestrator.Bind(secKeyHold, "WalkSpeed_KeyHold",       "Toggle")
    Orchestrator.Bind(secSpeed,   "WalkSpeed_Speed",         "Slider")
    Orchestrator.Bind(secTrail,   "WalkSpeed_GhostTrail",    "Toggle")
    Orchestrator.Bind(secTrail,   "WalkSpeed_DriftStrength", "Slider")
    -- =========================================================================

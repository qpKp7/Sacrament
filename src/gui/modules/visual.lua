...
function VisualModuleFactory.new(): VisualModule
    local maid = Maid.new()
            ...
            task.defer(function()
                for _, desc in ipairs(item.subFrame:GetDescendants()) do
                    if desc:IsA("ScrollingFrame") then
                        ...
                        if layout then
                            layout.Padding = UDim.new(0, 15) -- Mantido alinhado ao SubFrame Core
...
function VisualModuleFactory.createVisualFrame(parent: Frame): Frame
    local visualMod = VisualModuleFactory.new()
    visualMod.Instance.Parent = parent
    return visualMod.Instance
end

return VisualModuleFactory
